...
class Variable(object):
    ...
    def derivative(self, v):
        ...
        if self is v: 
            der = 1
        elif self.operands is None and self.d_op is None:
            der = 0
        elif self.operands is not None and self.d_op is not None:
            der = self.d_op(self.operands, v)
        else:
            raise(Exception("Neither dependent nor independent Variable"))
        return der
